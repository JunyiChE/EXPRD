...
def main(obj):
    ...
    def loadEid(event,id,y):
        ...
        if len(event) < 2:
            return None
        if len(event)>1:
            SourceTweetPath = os.path.join(cwd, 'data/' + obj + '/source_tweets.txt')
            not_found_count = 0
            # Search for the source tweet based on ID
            source_tweet = None
            with open(SourceTweetPath, 'r', encoding='utf-8') as file:
                for line in file:
                    tweet_id, tweet_text = line.split('\t', 1)
                    if tweet_id == id:
                        source_tweet = tweet_text.strip()
                        break
            if source_tweet is None:
                not_found_count += 1
                print("tweet not found: ", not_found_count)
                return None
            ...
            if rootindex == 0:
                ...
                x_pos[idx] = x_pos[idx_shuffle]
                                                                    # x_pos not used
            np.savez( os.path.join(cwd, 'data/'+obj+'graph/'+id+'.npz'), x=x_x,x_pos=x_pos,root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y,source_tweet=source_tweet, eid=id)
